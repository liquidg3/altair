define("altair/test", ["dojo/_base/lang"], function(lang){
...
//
    ...
    doh._runRegFixture = function(/*String*/ groupName, /*Object*/ fixture, method){
        // summary:
        ...
        if(ret && ret.then){
            ...
            var retEnd = function(){
                ...
                tg.inFlight--;
                dfd.resolve();
//moved to _runFixture where it all belongs
//                if(fixture.tearDown){
//                    try {
...
//                }
    ...
    doh._runFixture = function(groupName, fixture){
        ...
        var threw = false;
        var err = null;
        var name = fixture.name || 'unnammed test';
        var dfd = new doh.Deferred();
        var otherwise = function (e) {
            threw = true;
            err = e;
        };
            ...
            dfd = dfd.then(function () {
                ...
                if(fixture["runTest"]){		// should we error out of a fixture doesn't have a runTest?
                    ...
                    if(fixture.testType === "perf"){
                        ...
                        return doh._runRegFixture(groupName, fixture);
                    }
                }

            }.bind(this));

            //otherwise has to go before always
            dfd.otherwise(otherwise);

            //run teardown
            if(fixture.tearDown) {

                dfd = dfd.always(function () {
                    this.debug('TEARING DOWN: "' + name + '"');
                    return fixture.tearDown();
                }.bind(this)).otherwise(otherwise);
